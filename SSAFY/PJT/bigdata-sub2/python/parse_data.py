...
def create_movies():
    ...
    response = requests.post(API_URL + 'movie/get/', data=json.dumps(request_data), headers=headers)
    print(response.text)
...
def create_ratings(num_users):
    ...
    for line in rating_data.readlines():
        ...
        if len(request_data['ratings']) >= num_users:
            break
    response = requests.post(API_URL + 'movie/get/ratings/', data=json.dumps(request_data), headers=headers)
    print(response.text)
